...
リポジトリ内の `tasks/last-task.md` を読み込む。ファイルが存在する場合は1行目の `タスクリスト — YYYY-MM-DD` から日付を抽出し `last_date` とする。ファイルが存在しない場合は `last_date = 昨日の日付` とする。
...
`tasks/last-task.md` を読み込み、以下を明示的に分けて抽出し、後続ステップで使用する:
...
```bash
cp /tmp/task-{TODAY}.md tasks/last-task.md
git add tasks/last-task.md
git commit -m "chore: update last-task.md ({TODAY})"
git push
```
...
- 前回ファイルが見つからない場合は `last_date = 昨日` として処理する
