...
3. **手書き修正の優先反映（必須）**: `pending_tasks` の行テキストは、ユーザーが前回ノート上で直接書き換えている場合がある（例: 期限を「9/1」から「9/2」に訂正、内容の一部を訂正・追記）。この行のテキストは**現時点でユーザーが確定させた最新の内容**として扱い、そのまま（要約・統合する場合も訂正箇所は必ず残したまま）持ち越す。今回あらためて収集した Chatwork/Gmail/Slack の情報がこれと矛盾していても、ユーザーの手書き訂正を無断で古い内容へ戻さない。取得データにより訂正がさらに必要と判断できる場合のみ、手書き訂正後の文言をベースに追記更新する（例:「（ユーザー訂正後の内容）／さらに更新: ...」のように、訂正を消さず積み増す）。
4. **最終検算（出力前・必須）**: `pending_tasks` の件数と、本日のチェックリストに残った（(a)(b)で除外した以外の）件数を突き合わせる。数が合わない場合は、どの項目が消えたかを特定し、(a)(b)に該当しない限りチェックリストに戻してから出力する。
...
- **前回ノートの `pending_tasks` にユーザーが手書きで加えた訂正（期限の書き換え、内容の修正・追記など）は最新の正として扱い、そのまま持ち越す。今回収集したChatwork/Gmail/Slackの情報が古い内容と一致していても、手書き訂正を無断で古い内容に戻さない（Step 5-A0-3）**
